...
public class DepartmentInfoManagerImpl implements DepartmentInfoManager{
	...
	public List<DepartmentInfo> listGxDepInfo() throws WEBException {
		// TODO Auto-generated method stub
		DepartmentInfoDao dDao = null;
		try {
			dDao = (DepartmentInfoDao) DaoFactory.instance(null).getDao(Constants.DAO_DEP_INFO);
			Session sess = HibernateUtil.currentSession();
			return dDao.findGxDepInfo(sess);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			throw new WEBException("获取工序部门信息时出现异常!");
		} finally{
			HibernateUtil.closeSession();
		}
	}

	public List<DepartmentInfo> listSpecDepInfo(Integer dOption)
			throws WEBException {
		// TODO Auto-generated method stub
		DepartmentInfoDao dDao = null;
		try {
			dDao = (DepartmentInfoDao) DaoFactory.instance(null).getDao(Constants.DAO_DEP_INFO);
			Session sess = HibernateUtil.currentSession();
			return dDao.findSpecDepInfo(sess,dOption);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			throw new WEBException("根据情况获取工序部门信息时出现异常!");
		} finally{
			HibernateUtil.closeSession();
		}
	}
