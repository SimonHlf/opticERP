...
public class InstoreSubInfoManagerImpl implements InstoreSubInfoManager{
	...
	public List<InStoreSubInfo> listInfoByIssId(Integer issId)
			throws WEBException {
		// TODO Auto-generated method stub
		try {
			issDao = (InstoreSubInfoDao) DaoFactory.instance(null).getDao(Constants.DAO_IN_STORE_SUB_INFO);
			Session sess = HibernateUtil.currentSession();
			return issDao.findInfoByIssId(sess, issId);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			throw new WEBException("获取指定主键的入库详细信息列表异常");
		} finally{
			HibernateUtil.closeSession();
		}
	}

	public List<InStoreSubInfo> listInfoByProId(Integer proId)
			throws WEBException {
		// TODO Auto-generated method stub
		try {
			issDao = (InstoreSubInfoDao) DaoFactory.instance(null).getDao(Constants.DAO_IN_STORE_SUB_INFO);
			Session sess = HibernateUtil.currentSession();
			return issDao.findInfoByProId(sess, proId);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			throw new WEBException("根据材料编号获取加工剩余数量和入库数量相同的信息记录列表异常");
		} finally{
			HibernateUtil.closeSession();
		}
	}
