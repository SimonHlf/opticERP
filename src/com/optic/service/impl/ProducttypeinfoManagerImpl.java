...
public class ProducttypeinfoManagerImpl implements ProducttypeinfoManager {
	...
	public Integer addProductTypeInfo(String typeName, String typeRemark,String tPy)
			throws WEBException {
		try {
	...
	public boolean updateProductTypeInfo(Integer id, String typeName,
			String typeRemark,String tPy) throws WEBException {
		try {
			productTypeInfoDao = (ProductTypeInfoDao) DaoFactory.instance(null).getDao(Constants.DAO_PRODUCT_TYPE_INFO);
			Session sess = HibernateUtil.currentSession();
			tran = sess.beginTransaction();
	...
	public boolean checkExistByName(String typeName) throws WEBException {
		// TODO Auto-generated method stub
		try {
			productTypeInfoDao = (ProductTypeInfoDao) DaoFactory.instance(null).getDao(Constants.DAO_PRODUCT_TYPE_INFO);
			Session sess = HibernateUtil.currentSession();
			return productTypeInfoDao.checkExistByName(sess, typeName);
		} catch (Exception e) {
			throw new WEBException("检查是否存在该类别信息时出现异常,请重试！");
		} finally{
			HibernateUtil.closeSession();
		}
	}
